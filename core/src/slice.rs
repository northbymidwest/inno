...
/// # Panics
///
/// Panics if `slices_per_disk` is zero, which no installer records: the
/// header reader substitutes one.
#[must_use]
pub fn filename(base: &str, slice: u32, slices_per_disk: u32) -> String {
    ...
    let letter = char::from(b'a' + u8::try_from(slice % slices_per_disk).unwrap_or(b'z' - b'a'));
...
impl Slices {
    ...
    #[must_use]
    pub fn beside_named(installer: &Path, base: Option<&str>, slices_per_disk: u32) -> Self {
        let mut slices = Self::beside(installer, slices_per_disk);
        // A base name out of the installer's own header is not a path, and
        // must not become one: it is used to build a filename next to the
        // installer, nowhere else.
    ...
    #[must_use]
    pub fn beside(installer: &Path, slices_per_disk: u32) -> Self {
        Self {
            directory: installer
                .parent()
            ...
            base: installer
                ...
                .into_owned(),
            alternate: None,
            slices_per_disk: slices_per_disk.max(1),
            open: None,
        }
    }
    ...
    pub fn path(&self, slice: u32) -> PathBuf {
        ...
        match &self.alternate {
            ...
            // Reported against the name that was expected, rather than one
...
impl Read for Slices {
    fn read(&mut self, buffer: &mut [u8]) -> io::Result<usize> {
        ...
        let open = match self.open(next) {
            ...
            // Reported as an error, an ordinary end of stream throws, and
            // every reader that finishes by asking once more and expecting
            // nothing fails on the last chunk of a slice. A chunk that really
            // is cut short still fails, in the layer that knows how much it
            // was owed.
...
mod tests {
    ...
    use super::{HEADER_SIZE, Slices, filename, read_header};
    ...
    fn slice_on_disk(content: &[u8], slice: u32) -> (tempfile::TempDir, std::path::PathBuf) {
        ...
        let mut file =
            std::fs::File::create(directory.path().join(filename("setup_game", slice, 1)))
                .expect("create slice");
    ...
    fn data_is_read_from_the_offset_the_installer_records() {
        ...
        let mut slices = Slices::beside(&installer, 1);
    ...
    fn the_name_out_of_the_header_is_used_when_the_file_has_been_renamed() {
        ...
        let mut slices = Slices::beside_named(&renamed, Some("setup_game"), 1);
    ...
    fn a_name_out_of_the_header_cannot_reach_another_directory() {
        ...
        let directory = tempfile::tempdir().expect("temp dir");
        let installer = directory.path().join("setup_game.exe");
        let slices = Slices::beside_named(&installer, Some("../../etc/passwd"), 1);
    ...
    fn one_slice_per_disk_counts_from_one() {
        ...
        // filenames from one.
        assert_eq!(filename("setup_game", 0, 1), "setup_game-1.bin");
        assert_eq!(filename("setup_game", 1, 1), "setup_game-2.bin");
        assert_eq!(filename("setup_game", 9, 1), "setup_game-10.bin");
    }

    #[test]
    fn several_slices_per_disk_are_lettered_within_a_numbered_disk() {
        assert_eq!(filename("game", 0, 3), "game-1a.bin");
        assert_eq!(filename("game", 2, 3), "game-1c.bin");
        assert_eq!(filename("game", 3, 3), "game-2a.bin");
        assert_eq!(filename("game", 5, 3), "game-2c.bin");
    }
    ...
    fn a_name_with_its_own_punctuation_is_left_alone() {
        ...
        assert_eq!(
            filename("setup_rollercoaster_tycoon_2_(76932)", 0, 1),
