...
/// Where an installer's file data is read from.
///
/// An installer either keeps its data in the setup executable, after a header
/// whose length it records as the data offset, or in numbered `.bin` slices
/// beside it. The offsets in a file entry mean the same thing in both cases,
/// so the only difference is which file to look in, and this is that
/// difference.
...
impl<R: Read + Seek> DataSource for Embedded<R> {
    fn seek_to(&mut self, slice: u32, offset: u64) -> io::Result<()> {
        if slice > 0 {
            // Worth saying plainly. Reading on regardless gives whatever
            // happens to sit at that offset in the executable, which is how
            // this reads as a corrupt archive rather than as a file that was
            // never opened.
...
mod tests {
    ...
    fn asking_an_embedded_installer_for_a_later_slice_says_what_is_missing() {
        // Seeking anyway lands somewhere arbitrary in the executable and the
        // failure surfaces much later, as a chunk that will not decompress.
